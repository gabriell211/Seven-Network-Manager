...
impl DiscoveryStore {
    ...
    pub async fn list_scopes(
        ...
        let rows = sqlx::query(SCOPE_LIST_SQL)
    ...
    pub async fn get_scope(
        ...
        let row = sqlx::query(SCOPE_ONE_SQL)
            .bind(organization_id)
            .bind(site_id)
            .bind(scope_id)
    ...
    pub async fn create_scope(
        ...
        insert_scope(&mut tx, context, scope_id, &input).await?;
        ...
        write_mutation_records(
            ...
            context,
            MutationRecord {
                action: "discovery.scope.create",
                event_type: "discovery.scope.created",
                resource_type: "discovery_scope",
                resource_id: scope_id,
                before: None,
                after: Some(serde_json::to_value(&created)?),
            },
        )
        .await?;
        tx.commit().await?;
    ...
    pub async fn update_scope(
        ...
        update_scope_row(&mut tx, context, scope_id, &input.values).await?;
        ...
        write_mutation_records(
            ...
            context,
            MutationRecord {
                action: "discovery.scope.update",
                event_type: "discovery.scope.updated",
                resource_type: "discovery_scope",
                resource_id: scope_id,
                before: Some(before),
    ...
    pub async fn delete_scope(
        ...
        let deleted = sqlx::query(
            "DELETE FROM discovery_scopes WHERE id = $1 AND organization_id = $2 AND site_id = $3",
        )
        .bind(scope_id)
        ...
        if deleted.rows_affected() != 1 {
            return Err(DiscoveryStoreError::ScopeNotFound);
        }
        write_mutation_records(
            &mut tx,
            context,
            MutationRecord {
                action: "discovery.scope.delete",
                event_type: "discovery.scope.deleted",
                resource_type: "discovery_scope",
                resource_id: scope_id,
                before: Some(before),
                after: None,
    ...
    pub async fn create_run(
        ...
        for target in &seed_targets {
            if !scope.network.contains(target) {
                return Err(DiscoveryStoreError::SeedOutsideScope(*target));
            }
        }
        if active_run_exists(&mut tx, scope_id).await? {
            return Err(DiscoveryStoreError::ScopeHasActiveRun);
        }
        ...
        .execute(&mut *tx)
        .await?;
        advance_scope_schedule(&mut tx, scope_id).await?;
        let run = load_run_in_tx(
            &mut tx,
            context.organization_id,
            context.site_id,
            run_id,
        ...
        write_mutation_records(
            ...
            context,
            MutationRecord {
                action: "discovery.run.enqueue",
                event_type: "discovery.run.queued",
                resource_type: "discovery_run",
                resource_id: run_id,
                before: None,
                after: Some(serde_json::to_value(&run)?),
            },
        )
        .await?;
        tx.commit().await?;
    ...
    pub async fn claim_next_queued_run(
        ...
        let run = load_run_by_id_in_tx(&mut tx, run_id, false)
            .await?
            .ok_or(DiscoveryStoreError::RunNotFound)?;
        let scope = serde_json::from_value::<DiscoveryScopeView>(run.scope_snapshot.clone())
    ...
    pub async fn cancellation_requested(&self, run_id: Uuid) -> Result<bool, DiscoveryStoreError> {
        sqlx::query_scalar::<_, bool>(
            "SELECT cancellation_requested_at IS NOT NULL FROM discovery_runs WHERE id = $1",
        )
        .bind(run_id)
        .fetch_optional(&self.pool)
        .await?
    ...
    pub async fn request_cancel(
        ...
        write_mutation_records(
            ...
            context,
            MutationRecord {
                action: "discovery.run.cancel.request",
                event_type: "discovery.run.cancel_requested",
                resource_type: "discovery_run",
                resource_id: run_id,
                before: Some(before),
                after: Some(serde_json::to_value(&updated)?),
    ...
    pub async fn record_probe_result(
        ...
        if run.organization_id != result.target.organization_id.0
            || run.site_id != result.target.site_id.0
            || run.routing_domain_id != result.target.routing_domain_id.0
        {
            return Err(DiscoveryStoreError::ProbeScopeMismatch);
        }
        for item in &result.evidence {
            item.validate().map_err(DiscoveryStoreError::InvalidInput)?;
        }
        ...
        for item in &result.evidence {
            ...
            .bind(&item.value)
            .bind(&item.source)
            .bind(item.confidence.to_string())
        ...
        sqlx::query(
            r#"
            UPDATE discovery_runs
            SET progress_completed = LEAST(
                  progress_total,
                  (SELECT count(*)::integer FROM discovery_probe_results WHERE discovery_run_id = $1)
                ),
                updated_at = now(), version = version + 1
    ...
    pub async fn finish_run(
        ...
        write_mutation_records(
            ...
            &context,
            MutationRecord {
                action: "discovery.run.finish",
                event_type: "discovery.run.finished",
                resource_type: "discovery_run",
                resource_id: run_id,
...
impl NewDiscoveryScope {
    fn validate(&self, context: &MutationContext) -> Result<(), DiscoveryStoreError> {
        ...
        if self.tcp_ports.len() > 64 || self.tcp_ports.contains(&0) {
        ...
        if let Some(source) = self.source_address
            && !self.network.contains(&source)
        {
            return Err(DiscoveryStoreError::InvalidInput(
                "source address must belong to discovery CIDR",
            ));
        }
        Ok(())
    }
...
pub(crate) async fn load_scope_in_tx(
    ...
    let sql = if for_update {
        format!("{SCOPE_ONE_SQL} FOR UPDATE")
    } else {
        SCOPE_ONE_SQL.to_owned()
    };
...
SELECT s.id, s.organization_id, s.site_id, s.routing_domain_id,
       ...
       EXISTS (SELECT 1 FROM discovery_runs r WHERE r.scope_id = s.id AND r.status IN ('queued','running')) AS has_active_run
FROM discovery_scopes s
WHERE s.organization_id = $1 AND s.site_id = $2
ORDER BY s.name, s.network, s.id
"#;

const SCOPE_ONE_SQL: &str = r#"
SELECT s.id, s.organization_id, s.site_id, s.routing_domain_id,
       s.network::text AS network, s.ipv6_strategy, s.concurrency_limit,
       s.rate_per_second, s.timeout_ms, s.enabled, s.version, s.name,
       s.observations, s.tcp_ports, s.interface_scope, host(s.source_address) AS source_address,
...
async fn insert_scope(
    ...
    sqlx::query(
        ...
        INSERT INTO discovery_scopes (
          ...
          CASE WHEN $10 AND $17::integer IS NOT NULL THEN now() + make_interval(secs => $17) ELSE NULL END
        )
        "#,
    )
    .bind(scope_id)
            ...
            next_run_at = CASE
              WHEN $10 AND $17::integer IS NOT NULL THEN
                CASE WHEN schedule_interval_seconds IS DISTINCT FROM $17 OR next_run_at IS NULL
                     THEN now() + make_interval(secs => $17) ELSE next_run_at END
              ELSE NULL
            END,
...
async fn active_run_exists(
    ...
    Ok(sqlx::query_scalar::<_, bool>(
        "SELECT EXISTS (SELECT 1 FROM discovery_runs WHERE scope_id = $1 AND status IN ('queued','running'))",
    )
    .bind(scope_id)
    .fetch_one(&mut **tx)
    .await?)
...
async fn advance_scope_schedule(
    tx: &mut Transaction<'_, Postgres>,
    scope_id: Uuid,
) -> Result<(), DiscoveryStoreError> {
    sqlx::query(
...
fn row_to_scope(row: sqlx::postgres::PgRow) -> Result<DiscoveryScopeView, DiscoveryStoreError> {
    let network = row.try_get::<String, _>("network")?;
    let observations = row.try_get::<Vec<String>, _>("observations")?;
    let ports = row.try_get::<Vec<i32>, _>("tcp_ports")?;
...
fn row_to_run(row: sqlx::postgres::PgRow) -> Result<DiscoveryRunView, DiscoveryStoreError> {
    let seeds = row.try_get::<Vec<String>, _>("seed_targets")?;
    Ok(DiscoveryRunView {
        id: row.try_get("id")?,
...
fn row_to_probe_result(row: sqlx::postgres::PgRow) -> Result<ProbeResultView, DiscoveryStoreError> {
...
fn is_terminal(status: DiscoveryStatus) -> bool {
    matches!(
        status,
...
fn validate_page(limit: i64, offset: i64, max: i64) -> Result<(), DiscoveryStoreError> {
    if !(1..=max).contains(&limit) || offset < 0 {
        Err(DiscoveryStoreError::InvalidInput("pagination is invalid"))
    } else {
        Ok(())
    }
}
...
pub(crate) async fn write_mutation_records(
    tx: &mut Transaction<'_, Postgres>,
    context: &MutationContext,
    ...
    AuditStore::append_in_tx(
        ...
        AuditEventDraft {
            ...
            resource_type: record.resource_type.to_owned(),
            resource_id: Some(record.resource_id.to_string()),
            provider: None,
    ...
    OutboxStore::enqueue(
        ...
        &NewOutboxEvent {
            id: Uuid::now_v7(),
            organization_id: context.organization_id,
            aggregate_type: record.resource_type.to_owned(),
            aggregate_id: record.resource_id.to_string(),
...
mod tests {
    ...
    #[tokio::test]
    async fn repeated_probe_result_is_idempotent_for_progress() {
        let Some(pool) = pool().await else { return };
        ...
        let result = DiscoveryProbeResult {
            ...
            probe: snm_domain::discovery::DiscoveryProbeKind::Icmp,
            target: snm_domain::ScopedIpTarget {
                organization_id: OrganizationId(context.organization_id),
                site_id: SiteId(context.site_id),
                routing_domain_id: RoutingDomainId(routing_domain_id),
