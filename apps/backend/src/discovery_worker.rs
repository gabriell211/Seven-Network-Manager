...
use snm_domain::{
    ...
    discovery::{DiscoveryProbeRequest, DiscoveryProbeResult, DiscoveryStatus},
    inventory::{DeviceType, IdentifierKind},
    ipam::{AddressState, is_ipv6_link_local},
};
use snm_inventory_store::{
    DeviceObservation, InventoryStore, MutationContext as InventoryMutationContext, NewIdentifier,
};
use snm_ipam_store::{
...
impl DiscoveryWorker {
    ...
    async fn execute_run(&self, input: RunExecutionInput) -> Result<(), String> {
        let run = &input.run;
        let actor = run.requested_by.unwrap_or(Uuid::nil());
        ...
        let executor = Arc::new(RuntimeProbeExecutor {
            runtime: self.runtime.clone(),
        });
        let sink = Arc::new(StoreResultSink {
            store: self.store.clone(),
            run_id: run.id,
        });
        let outcome = IncrementalDiscoveryOrchestrator::new(executor, sink)
        ...
        if let Err(error) = reconciliation {
            ...
            merge_summary(&mut summary, "reconciliationError", json!(error));
        }
    ...
    async fn reconcile(
        ...
        for (address, host) in hosts {
            ...
            if !is_ipv6_link_local(address) {
                if let Some((prefix, _)) = parsed_prefixes
                    .iter()
